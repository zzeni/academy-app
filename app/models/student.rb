class Student < ApplicationRecord
  ...
  def attend(course)
    raise CourseFullError if course.complete?

    courses_in_the_same_category = courses.where(category_id: course.category_id)
    raise TooManyCoursesAtATimeError if courses.size >= 2

    max_lvel_for_category = courses_in_the_same_category.maximum(:level)
    raise NotEligibleForCourseError if course.level - 1 > (max_lvel_for_category||0)
